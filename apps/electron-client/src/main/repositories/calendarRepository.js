...
class CalendarRepository {
   ...
   * @param {String} calendarData.schedule - 일정 내용
  ...
  async saveCalendarEntry(calendarData) {
    try {
      ...
      const { message_id, account_id, schedule, task } = calendarData;
      ...
      return new Promise((resolve, reject) => {
        ...
        const query = `
          INSERT OR REPLACE INTO Calendar (message_id, account_id, schedule, task)
          VALUES (?, ?, ?, ?)
        `;

        db.run(query, [message_id, account_id, schedule, task], function (err) {
          if (err) {
  ...
  async getCalendarEntryByMessageId(message_id) {
    try {
      ...
      return new Promise((resolve, reject) => {
        const query = `
          SELECT message_id, account_id, schedule, task
          FROM Calendar
          WHERE message_id = ?
        `;
